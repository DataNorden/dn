 ...
 * ```
 */
export function OptionsConfigurator<P extends Record<string, TSchema>>(
  schemaOrProps: P | TObject<P>,
) {
      ...
      if ('default' in prop) {
        Reflect.set(
          ...
          (typeof prop.default === 'function'
            ? prop.default
            : () => prop.default) as () => Options[keyof Options],
        )
      }
      return acc
    },
...
>(options: {
  ...
  return function configurator() {
    const ins = new CFG(Object.create(null))
...
> = {
  [K in keyof Required<T>]: (
    value: T[K],
  ) => TConfigurator<Omit<T, K>, SetFieldType<O, K, T[K]>>
}
